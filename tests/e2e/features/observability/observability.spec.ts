...
test.describe("Observability", () => {
  ...
  test.describe("Navigation", () => {
    test("should display observability page", async ({ observabilityPage }) => {
      ...
      const providersHeader = observabilityPage.page
  ...
  test.describe("OTel Connector", () => {
    ...
    test("should display metrics endpoint", async ({ observabilityPage }) => {
      ...
      const metricsInput = observabilityPage.page.getByPlaceholder(
        /v1\/metrics|otel-collector.*metrics/i,
      );
    ...
    test("should display OTel delete button when connector is configured", async ({
      observabilityPage,
    }) => {
      await observabilityPage.selectConnector("otel");
    ...
    test("should expose the latest ordered selective export policy editor", async ({
      observabilityPage,
    }) => {
      await observabilityPage.selectConnector("otel");
  ...
  test.describe("Maxim Connector", () => {
    ...
    test("should display Maxim configuration form", async ({ observabilityPage }) => {
      ...
      if (formVisible) {
        const hasInputs = await form
          .locator("input")
          .first()
          .isVisible()
          .catch(() => false);
        const hasSwitches = await form
        ...
        const inputsVisible = await observabilityPage.page
          .locator("input")
          .first()
          .isVisible()
          .catch(() => false);
  ...
  test.describe("Prometheus Connector", () => {
    ...
    test("should display Prometheus configuration when available", async ({
      observabilityPage,
    }) => {
      const isAvailable = await observabilityPage.isConnectorAvailable("prometheus");
    ...
    test("should display Prometheus delete button when connector is configured", async ({
      observabilityPage,
    }) => {
      const isAvailable = await observabilityPage.isConnectorAvailable("prometheus");
