...
function VKAssignedToCell({ vk }: { vk: VirtualKey }) {
	...
	if (!label) {
		return (
			<span className="text-muted-foreground max-w-full truncate text-left text-sm">{i18n.t("workspace.routingRules.notAvailable")}</span>
		);
	}
...
function VKActiveSwitch({
	...
	return (
		<Switch
			...
			aria-label={`${vk.is_active ? i18n.t("common.disable") : i18n.t("common.enable")} virtual key ${vk.name}`}
			data-testid={`vk-active-switch-${vk.name}`}
			title={isManagedByProfile ? i18n.t("workspace.virtualKeys.managedByProfileShort") : undefined}
			...
			<DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
				<DropdownMenuTrigger asChild>
					<Button
						...
						size="icon"
						className="h-8 w-8"
						aria-label={i18n.t("workspace.virtualKeys.actionsAria")}
				...
				<DropdownMenuContent align="end">
					...
					<DropdownMenuItem
						...
						title={isManagedByProfile ? i18n.t("workspace.virtualKeys.managedDeleteDisabled") : undefined}
						onSelect={(e) => {
							e.preventDefault();
							setDeleteOpen(true);
							setIsOpen(false);
			...
			<AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
				<AlertDialogContent>
					...
					<AlertDialogFooter>
						...
						>
							{isDeleting ? i18n.t("workspace.virtualKeys.deleteDialogDeleting") : i18n.t("common.delete")}
						</AlertDialogAction>
					</AlertDialogFooter>
				</AlertDialogContent>
			</AlertDialog>
...
export default function VirtualKeysTable({
	...
	const handleToggleActive = async (vk: VirtualKey, checked: boolean) => {
		try {
			...
			toast.success(
				`Virtual key ${checked ? i18n.t("workspace.providers.keyTable.itemEnabled") : i18n.t("workspace.providers.keyTable.itemDisabled")}`,
			);
		} catch (error) {
			toast.error(getErrorMessage(error));
			throw error;
	...
	return (
		<>
						...
						<div className="space-y-2">
							...
							<div className="grid grid-cols-1 gap-2 sm:grid-cols-2" data-testid="vk-export-scope">
								...
								>
									...
									<span className="text-muted-foreground text-xs">
										{totalCount} {i18n.t("workspace.virtualKeys.total")}
									</span>
								</button>
							</div>
						</div>
						...
						{exportScope === "all" && (
							<div className="space-y-2">
								<Label htmlFor="export-max-limit" className="text-sm">
									{i18n.t("workspace.virtualKeys.maxEntriesOptional")}
								</Label>
								<Input
									id="export-max-limit"
									type="number"
			...
			<AlertDialog open={showBulkRotateDialog} onOpenChange={setShowBulkRotateDialog}>
				<AlertDialogContent>
					<AlertDialogHeader>
						<AlertDialogTitle>{i18n.t("workspace.virtualKeys.bulkRotateTitle")}</AlertDialogTitle>
						<AlertDialogDescription>
							{i18n.t("workspace.virtualKeys.bulkRotateDescription", {
								count: selectedCount,
							})}
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
						...
						>
							{isBulkRotating ? i18n.t("workspace.virtualKeys.rotating") : i18n.t("workspace.virtualKeys.rotateSelected")}
						</AlertDialogAction>
					</AlertDialogFooter>
				</AlertDialogContent>
			</AlertDialog>
			...
			<div className="flex min-h-0 w-full grow flex-col overflow-hidden">
				...
				<div className="mb-4 flex shrink-0 flex-wrap items-center gap-3">
					<PageTitle title={i18n.t("sidebar.sub.virtualKeys")}>{i18n.t("workspace.virtualKeys.tableDescription")}</PageTitle>
					...
					{UserPicker && (
						<div className="flex w-full min-w-0 items-center gap-1 sm:w-auto sm:max-w-[250px] sm:flex-1" data-testid="vk-user-filter">
							<UserPicker
								...
								placeholder={i18n.t("workspace.virtualKeys.allUsers")}
								triggerClassName="h-9"
								className="w-full min-w-0"
							/>
							<FilterClearButton
					...
					<div className="flex w-full flex-wrap items-center gap-2 sm:ml-auto sm:w-auto sm:shrink-0 sm:flex-nowrap">
						{selectedCount > 0 && (
							...
							>
								...
								{i18n.t("workspace.virtualKeys.rotateSelectedCount", {
									count: selectedCount,
								})}
							</Button>
						)}
							...
							<TableRow>
								<TableHead className="w-[48px]">
									<Checkbox
										...
										aria-label={i18n.t("workspace.virtualKeys.selectAllPageAria")}
										data-testid="vk-select-all-checkbox"
									/>
								</TableHead>
								<TableHead className="w-[250px]">
									<SortableHeader column="name" label={i18n.t("workspace.virtualKeys.columnName")} />
								</TableHead>
								<TableHead className="w-[160px]">{i18n.t("workspace.virtualKeys.assignedTo")}</TableHead>
								<TableHead className="w-[440px]">{i18n.t("workspace.virtualKeys.columnKey")}</TableHead>
								<TableHead className="w-[200px]">
									<SortableHeader column="budget_spent" label={i18n.t("workspace.virtualKeys.columnBudget")} />
								</TableHead>
								<TableHead className="w-[200px]">{i18n.t("workspace.virtualKeys.rateLimits")}</TableHead>
								<TableHead className="w-[120px]">
									<SortableHeader column="status" label={i18n.t("workspace.virtualKeys.columnStatus")} />
								</TableHead>
