					...
					<div className="space-y-4">
						...
						<div className="grid gap-4">
							...
							{virtualKey.expires_at && (
								<div className="grid grid-cols-1 items-center gap-4 md:grid-cols-3">
									<span className="text-muted-foreground text-sm">{i18n.t("workspace.virtualKeys.expires")}</span>
									...
									{virtualKey.provider_configs.map((config, index) => (
										<div key={`${config.provider}-${index}`} className="rounded-lg border p-4">
												...
												<div className="flex items-center gap-2">
													<Badge variant="outline" className="font-mono text-xs">
														...
														{config.weight != null ? (
															...
															<span className="text-muted-foreground italic">{i18n.t("workspace.virtualKeys.notSet")}</span>
														)}
													</Badge>
													{!isManagedByProfile ? (
											...
											<div className="space-y-3">
												...
												<div className="grid grid-cols-1 items-start gap-4 md:grid-cols-3">
													<span className="text-muted-foreground pt-0.5 text-sm font-medium">
														{i18n.t("workspace.providers.apiKeyForm.blockedModels")}
													</span>
													<div className="col-span-2">
																...
																<div key={bIdx} className="space-y-2">
																	...
																	{hasActiveBudgetOverride(b) ? (
																		<p className="text-muted-foreground text-xs">
																			{i18n.t("workspace.virtualKeys.baseOverride", {
													...
													<>
														...
														<div className="space-y-3">
															...
															{config.rate_limit.token_max_limit != null ? (
																<div className="space-y-2">
																	<span className="text-muted-foreground text-xs font-medium">
																		{i18n.t("workspace.virtualKeys.tokenLimits")}
																	</span>
																	<UsageLine
															...
															{config.rate_limit.request_max_limit != null ? (
																<div className="space-y-2">
																	<span className="text-muted-foreground text-xs font-medium">
																		{i18n.t("workspace.virtualKeys.requestLimits")}
																	</span>
																	<UsageLine
															...
															{config.rate_limit.token_max_limit == null && config.rate_limit.request_max_limit == null && (
																<p className="text-muted-foreground text-sm">{i18n.t("workspace.virtualKeys.noProviderRateLimits")}</p>
															)}
														</div>
													</>
												)}
												...
												{config.model_budgets && config.model_budgets.length > 0 && (
													<>
														<DottedSeparator />
														<div className="space-y-3">
															<h4 className="text-sm font-medium">{i18n.t("workspace.virtualKeys.modelBudgets")}</h4>
							...
							<h3 className="font-semibold">
								...
								{isManagedByProfile && managingProfile?.budgets?.length ? (
									<span className="text-muted-foreground ml-2 text-xs font-normal">
										{i18n.t("workspace.virtualKeys.fromAccessProfile", {
											name: managingProfile.name,
										})}
									</span>
								) : null}
							</h3>
										...
										{hasActiveBudgetOverride(b) ? (
											<p className="text-muted-foreground text-xs">
												...
												{b.override_mode === "cycles"
													? i18n.t("workspace.virtualKeys.overrideCyclesRemaining", { count: b.override_cycles_remaining })
													: i18n.t("workspace.virtualKeys.overrideUntilRemoved")}
											</p>
										) : null}
										<div className="text-muted-foreground flex items-center justify-between text-xs">
											<span>
