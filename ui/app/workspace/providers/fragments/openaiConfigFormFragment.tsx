...
export function OpenAIConfigFormFragment({ provider }: OpenAIConfigFormFragmentProps) {
	...
	const onSubmit = (data: OpenAIConfigFormSchema) => {
		updateProvider(
			...
			.unwrap()
			.then(() => {
				toast.success(t("workspace.providers.openaiConfigurationUpdated"));
				form.reset(data);
			})
			.catch((err) => {
				toast.error(t("workspace.providers.openaiConfigurationUpdateFailed"), {
		...
		<Form {...form}>
			<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 px-4 md:px-6" data-testid="provider-config-openai-content">
							...
							<FormItem>
								<div className="flex items-center justify-between space-x-2">
									<div className="space-y-0.5">
										<FormLabel>{t("workspace.providers.disableStore")}</FormLabel>
										<p className="text-muted-foreground text-xs">{t("workspace.providers.disableStoreDescription")}</p>
									</div>
									<FormControl>
										<Switch
				...
				<div className="flex justify-end space-x-2 pb-6">
					...
					>
						{t("workspace.providers.saveOpenAIConfiguration")}
					</Button>
				</div>
			</form>
		</Form>
