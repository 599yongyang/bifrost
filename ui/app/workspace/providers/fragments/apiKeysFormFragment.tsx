			...
			render={({ field }) => (
				<FormItem className="flex flex-row items-center justify-between rounded-sm border p-2">
					<div className="space-y-1.5">
						...
						<FormDescription>
							{i18n.t("workspace.providers.apiKeyForm.useForBatchApisDesc")}
						</FormDescription>
					</div>
					<FormControl>
		...
		<div data-tab="api-keys" className="space-y-4 overflow-hidden">
			...
			{!isAzure && !isBedrock && !isBedrockMantle && !isVertex && (
				<FormField
					...
					render={({ field }) => (
						<FormItem>
							<FormLabel>{i18n.t("workspace.virtualKeys.apiKey")} {isVLLM ? "(Optional)" : ""}</FormLabel>
							<FormControl>
					...
					{azureAuthType === "api_key" && (
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									<FormLabel>
										{i18n.t("workspace.virtualKeys.apiKey")} {isVertex ? "(Supported only for gemini and fine-tuned models)" : isVLLM ? "(Optional)" : ""}
									</FormLabel>
									<FormControl>
										<SecretVarInput placeholder={i18n.t("workspace.providers.apiKeyForm.apiKeyPlaceholder")} type="text" {...field} />
									</FormControl>
				...
				<div className="space-y-4">
					...
					<div className="space-y-2">
						...
						{vertexAuthType === "service_account" && (
							<p className="text-muted-foreground text-sm">
								{i18n.t("workspace.providers.apiKeyForm.serviceAccountAttachedDesc")}
							</p>
						)}
					</div>
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormControl>
										<SecretVarInput data-testid="apikey-vertex-api-key-input" placeholder={i18n.t("workspace.providers.apiKeyForm.apiKeyPlaceholder")} type="text" {...field} />
									</FormControl>
									<FormMessage />
								</FormItem>
							)}
						...
						render={({ field }) => (
							<FormItem className="flex flex-row items-center justify-between rounded-sm border p-2">
								<div className="space-y-1.5">
									...
									<FormDescription>
										{i18n.t("workspace.providers.apiKeyForm.useDeploymentsEndpointDesc")}
									</FormDescription>
								</div>
								<FormControl>
									<Switch checked={field.value ?? false} onCheckedChange={field.onChange} />
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								...
								<FormControl>
									<SecretVarInput data-testid="key-input-vllm-url" placeholder={i18n.t("workspace.providers.apiKeyForm.vllmUrlPlaceholder")} {...field} />
								</FormControl>
								<FormMessage />
							</FormItem>
						)}
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								...
								<FormControl>
									<Input data-testid="key-input-vllm-model-name" placeholder={i18n.t("workspace.providers.apiKeyForm.vllmModelNamePlaceholder")} {...field} />
								</FormControl>
								<FormMessage />
							</FormItem>
						)}
						...
						render={({ field }) => (
							<FormItem>
								...
								<FormDescription>
									...
									{isOllama ? "http://localhost:11434" : "http://localhost:30000"} {i18n.t("workspace.virtualKeys.or")} {isOllama ? "env.OLLAMA_URL" : "env.SGL_URL"})
								</FormDescription>
								<FormControl>
									<SecretVarInput
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										...
										<FormDescription>
											{i18n.t("workspace.providers.apiKeyForm.assumeRoleArnDesc")}
										</FormDescription>
										<FormControl>
											<SecretVarInput
