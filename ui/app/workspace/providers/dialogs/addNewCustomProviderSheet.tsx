	...
	return (
		<>
			<SheetHeader className="flex shrink-0 flex-col items-start px-4 py-4 md:px-8" headerClassName="mb-0 sticky -top-4 bg-card z-10">
				<SheetTitle>{i18n.t("workspace.providers.addCustom.title")}</SheetTitle>
				<SheetDescription>{i18n.t("workspace.providers.addCustom.description")}</SheetDescription>
			</SheetHeader>
			<Form {...form}>
				<form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col">
					<div className="min-h-0 flex-1 space-y-4 px-4 pb-4 md:px-8">
						<FormField
							...
							render={({ field }) => (
								<FormItem className="flex flex-col gap-3">
									<FormLabel className="text-right">{i18n.t("workspace.providers.addCustom.name")}</FormLabel>
									<div className="col-span-3">
										<FormControl>
											<Input
												placeholder={i18n.t("workspace.providers.addCustom.name")}
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem className="flex flex-col gap-3">
									<FormLabel>{i18n.t("workspace.providers.addCustom.baseFormat")}</FormLabel>
									<div>
										<FormControl>
											<Select onValueChange={field.onChange} value={field.value} disabled={!hasProviderCreateAccess}>
												<SelectTrigger className="w-full" data-testid="base-provider-select">
													<SelectValue placeholder={i18n.t("workspace.providers.addCustom.selectBaseFormat")} />
												</SelectTrigger>
												<SelectContent>
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem className="flex flex-col gap-3">
									<FormLabel>{i18n.t("workspace.providers.baseUrl")}</FormLabel>
									<div>
										<FormControl>
											<Input
									...
									<div className="flex items-center justify-between space-x-2 rounded-lg border p-3">
										<div className="space-y-0.5">
											<label htmlFor="allow-private-network" className="text-sm font-medium">
												{i18n.t("workspace.providers.allowPrivateNetwork")}
											</label>
											<p className="text-muted-foreground text-sm">{i18n.t("workspace.providers.allowPrivateNetworkDescription")}</p>
										</div>
										<Switch
