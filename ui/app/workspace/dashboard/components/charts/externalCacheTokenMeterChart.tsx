		...
		<ChartErrorBoundary resetKey={`${data?.buckets?.length ?? 0}-${totalCachedRead}-${totalPromptTokens}`}>
			<div className="grid h-full grid-rows-[104px_auto] items-start overflow-hidden pt-8">
				<div ref={ref} className="relative h-full w-full grow">
					{!hasData && <div className="text-muted-foreground flex h-full items-center justify-center text-sm">{i18n.t("common.noDataAvailable")}</div>}
					...
					<div>
						<div className="flex shrink-0 flex-col items-center pt-1 leading-none">
							...
							<div className="mt-1 flex items-center gap-1 text-[11px] text-zinc-400">
								<span>{i18n.t("workspace.dashboard.externalCacheRate")}</span>
								<Tooltip>
									<TooltipTrigger asChild>
										<button
											...
											aria-label={i18n.t("workspace.dashboard.externalCacheInfoAriaLabel")}
										>
											<Info className="h-3 w-3" />
										</button>
									</TooltipTrigger>
									<TooltipContent side="top">{i18n.t("workspace.dashboard.externalCacheInfoTooltip")}</TooltipContent>
								</Tooltip>
							</div>
						</div>
						<div className="flex shrink-0 flex-wrap items-center justify-center gap-x-4 gap-y-1 pt-2 text-[11px] leading-none">
							...
							<span className="flex items-center gap-1.5">
								...
								<span className="text-muted-foreground">{i18n.t("workspace.promptRepository.messages.input")} {formatCompactNumber(totalPromptTokens)}</span>
							</span>
						</div>
					</div>
				)}
