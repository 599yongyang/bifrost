		...
		>
			...
			>
				...
				>
					...
					<span className="text-muted-foreground ml-auto text-[10px]">{copy.targetCount(rule.targets.length)}</span>
				</div>
					...
					>
						...
						{rule.chain_rule && (
							<div className="mb-1 flex items-start gap-2 border-b px-3 pb-1.5">
								...
								<p className="text-muted-foreground text-[10px] leading-snug">{copy.chainRuleHelp}</p>
							</div>
						)}
						<p className="mb-1 px-3 text-[10px] font-semibold tracking-wide uppercase" style={{ color: scopeColor }}>
							{rule.chain_rule ? copy.resolvedTarget : copy.targets}
						</p>
								...
								<div key={i} className="hover:bg-muted flex items-center gap-2 px-3 py-1.5">
									...
									<div className="min-w-0 flex-1">
										<p className="text-foreground truncate text-xs font-medium">
											{isPassthrough ? copy.passthrough : t.provider ? getProviderLabel(t.provider) : t.model}
										</p>
										{t.model && t.provider && <p className="text-muted-foreground truncate font-mono text-[10px]">{t.model}</p>}
										{isPassthrough && <p className="text-muted-foreground/60 text-[10px] italic">{copy.originalProviderModel}</p>}
									</div>
									{multi && (
