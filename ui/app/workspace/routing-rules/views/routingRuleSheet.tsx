...
import { ErrorFallbackEditor } from "@/app/workspace/routing-rules/components/errorFallbackEditor";
...
export function RoutingRuleSheet({ open, onOpenChange, editingRule, onSuccess }: RoutingRuleDialogProps) {
	...
	const availableProviders = Array.from(
		new Set([
			...
			...rules.flatMap((r) =>
				(r.error_fallbacks ?? []).flatMap((errorFallback) =>
					errorFallback.fallbacks.map((fallback) => fallback.split("/")[0]?.trim()).filter(Boolean),
				),
			),
			...(errorFallbacks ?? []).flatMap((errorFallback) =>
				errorFallback.fallbacks.map((fallback) => fallback.split("/")[0]?.trim()).filter(Boolean),
			),
		]),
	);
	...
	const onSubmit = (data: RoutingRuleFormData) => {
		...
		const errorFallbackForms = data.error_fallbacks || [];
		const errorFallbackValidation = validateErrorFallbackForms(errorFallbackForms);
		if (errorFallbackValidation.length > 0) {
			toast.error(errorFallbackValidation.join("\n"));
			return;
		}
		const normalizedErrorFallbacks = mergeContentSafetyErrorFallbacks(editingRule?.error_fallbacks || [], errorFallbackForms);
		...
		<Sheet open={open} onOpenChange={onOpenChange}>
			<SheetContent className="flex w-full min-w-1/2 flex-col gap-4 overflow-x-hidden p-0 pt-4">
				<SheetHeader className="flex flex-col items-start px-4 py-4 md:px-8" headerClassName="mb-0 sticky -top-4 bg-card z-10">
					...
					<SheetDescription>
						{isEditing ? copy.updateRuleDescription : copy.createRuleDescription}
					</SheetDescription>
				</SheetHeader>
				...
				<form onSubmit={handleSubmit(onSubmit)} className="flex grow flex-col">
					<div className="flex grow flex-col gap-6 px-4 pb-6 md:px-8">
						...
						<div className="space-y-3">
							<div className="flex items-center justify-between">
								<div>
									<Label>{copy.fallbacks}</Label>{" "}
									<p className="text-muted-foreground mt-0.5 text-xs">{copy.fallbacksHelp}</p>
								</div>
								<Button
									type="button"
									variant="outline"
						...
						<ErrorFallbackEditor
							value={errorFallbacks || []}
							providerOptions={providerOptions}
							onChange={(next) => setValue("error_fallbacks", next, { shouldDirty: true })}
						/>
					</div>
					{/* Action Buttons */}
...
function TargetRow({ target, index, providerOptions, allKeys, showRemove, onUpdate, onRemove }: TargetRowProps) {
	...
	return (
		<div className="space-y-3 rounded-lg border p-3" data-testid={`routing-target-${index}`}>
			<div className="flex items-center justify-between">
				<span className="text-muted-foreground text-sm font-medium">{copy.target} {index + 1}</span>
