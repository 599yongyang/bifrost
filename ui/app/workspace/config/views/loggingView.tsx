...
export default function LoggingView() {
	...
	const handleSave = useCallback(async () => {
		...
		try {
			...
			toast.error(t("workspace.config.logging.configurationUpdateFailed"), { description: getErrorMessage(error) });
		}
	}, [bifrostConfig, localConfig, t, updateCoreConfig]);

	return (
		<div className="mx-auto w-full max-w-4xl space-y-4 px-4 py-6 md:px-0">
			<PageTitle title={t("sidebar.sub.logsSettings")}>{t("workspace.config.logging.description")}</PageTitle>
			...
			<div className="space-y-4">
						...
						<div className="flex items-center justify-between space-x-2 rounded-sm border p-4">
							<div className="space-y-0.5">
								...
								<p className="text-muted-foreground text-sm">
									<Trans
										t={t}
										i18nKey="workspace.config.logging.disableContentLoggingDesc"
					...
					<div className="flex items-center justify-between space-x-2 rounded-sm border p-4">
						<div className="space-y-0.5">
							<label htmlFor="retain-content-in-object-storage" className="text-sm font-medium">
								{t("workspace.config.logging.retainContentInObjectStorage")}
							</label>
							<p className="text-muted-foreground text-sm">
								<Trans
									t={t}
									i18nKey="workspace.config.logging.retainContentInObjectStorageDesc"
									components={{ code: <code className="text-xs" /> }}
								/>
								{!bifrostConfig?.is_object_storage_connected && (
									<span className="text-destructive font-medium"> {t("workspace.config.logging.objectStorageRequired")}</span>
								)}
							</p>
						</div>
						<Switch
					...
					<div className="flex items-center justify-between space-x-2 rounded-sm border p-4">
						<div className="space-y-0.5">
							...
							<p className="text-muted-foreground text-sm">
								<Trans
									t={t}
									i18nKey="workspace.config.logging.perRequestContentOverrideDesc"
									components={{ code: <code className="text-xs" /> }}
								/>
							</p>
						</div>
				...
				<div className="flex items-center justify-between space-x-2 rounded-sm border p-4">
					<div className="space-y-0.5">
						...
						<p className="text-muted-foreground text-sm">
							<Trans
								t={t}
								i18nKey="workspace.config.logging.perRequestRawOverrideDesc"
								components={{ code: <code className="text-xs" /> }}
							/>
						</p>
					</div>
				...
				{localConfig.enable_logging && bifrostConfig?.is_logs_connected && (
					<div className="space-y-2 rounded-sm border p-4">
						...
						<p className="text-muted-foreground text-sm">
							<Trans t={t} i18nKey="workspace.config.logging.loggingHeadersDesc" components={{ code: <code className="text-xs" /> }} />
						</p>
						<Textarea
							id="logging-headers"
			...
			<div className="flex justify-end pt-2">
				<Button onClick={handleSave} disabled={!hasChanges || isLoading || !hasSettingsUpdateAccess}>
					{isLoading ? t("common.saving") : t("workspace.config.logging.saveChanges")}
				</Button>
			</div>
		</div>
	);
...
const RestartWarning = () => {
	const { t } = useTranslation();
	return <div className="text-muted-foreground mt-2 pl-4 text-xs font-semibold">{t("workspace.config.security.restartRequired")}</div>;
};