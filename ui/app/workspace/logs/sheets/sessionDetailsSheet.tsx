...
export function SessionDetailsSheet({
	...
	const summaryCards: SummaryCard[] = useMemo(
		() => [
			...
			{
				label: "Started",
				value: sessionSummary?.started_at
					? format(new Date(sessionSummary.started_at), "MMM d, yyyy hh:mm:ss aa")
					: i18n.t("workspace.mcpLogs.na"),
				size: "sm",
			},
			{
				label: "Latest Update",
				value: sessionSummary?.latest_at
					? format(new Date(sessionSummary.latest_at), "MMM d, yyyy hh:mm:ss aa")
					: i18n.t("workspace.mcpLogs.na"),
				size: "sm",
			},
			{
		...
		<Sheet open={open} onOpenChange={onOpenChange}>
			<SheetContent className="flex w-full flex-col gap-4 overflow-x-hidden p-4 sm:max-w-[60%] md:p-8">
				<div className="flex items-center justify-between gap-4">
					<div>
						<div className="text-lg font-medium">{i18n.t("workspace.logs.session.title")}</div>
					...
					<div className="flex items-center gap-3">
						...
						>
							...
							{sortOrder === "asc" ? i18n.t("workspace.logs.session.earliestFirst") : i18n.t("workspace.logs.session.latestFirst")}
						</Button>
					</div>
				</div>
										...
										<TableCell className="relative text-xs">
											{log.id === highlightedLogId ? (
												<div className="bg-background pointer-events-none absolute -top-1.5 left-1 z-10 rounded-full border border-sky-400/45 px-1.5 py-0 text-[9px] leading-tight font-semibold tracking-wide text-sky-600 uppercase dark:text-sky-300">
													{i18n.t("workspace.logs.session.current")}
												</div>
											) : null}
