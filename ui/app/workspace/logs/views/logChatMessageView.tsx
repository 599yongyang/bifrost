	...
	return (
		<div className="flex w-full flex-col gap-2">
			...
			<div className="flex items-center gap-2">
				...
				{message.tool_call_id && <span className="text-muted-foreground text-xs">{i18n.t("supplemental.toolCallId")} {message.tool_call_id}</span>}
			</div>
			...
			{message.reasoning && (
				<>
					{isJson(message.reasoning) ? (
						<CollapsibleBox title={i18n.t("workspace.logs.detail.reasoning")} onCopy={() => JSON.stringify(cleanJson(message.reasoning), null, 2)} collapsedHeight={100}>
							<CodeEditor
								className="z-0 w-full"
								shouldAdjustInitialHeight={true}
								maxHeight={200}
			...
			{message.refusal && (
				<>
					{isJson(message.refusal) ? (
						<CollapsibleBox title={i18n.t("workspace.logs.detail.refusalLabel")} onCopy={() => JSON.stringify(cleanJson(message.refusal), null, 2)} collapsedHeight={100}>
							<CodeEditor
								className="z-0 w-full"
								shouldAdjustInitialHeight={true}
								maxHeight={150}
			...
			{message.annotations && message.annotations.length > 0 && (
				<CollapsibleBox title={i18n.t("supplemental.annotations")} onCopy={() => JSON.stringify(message.annotations, null, 2)} collapsedHeight={100}>
					<CodeEditor
						className="z-0 w-full"
						shouldAdjustInitialHeight={true}
						maxHeight={400}
