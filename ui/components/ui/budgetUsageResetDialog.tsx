	...
	return (
		<AlertDialog open={open} onOpenChange={onOpenChange}>
			<AlertDialogContent data-testid={testId}>
				<AlertDialogHeader>
					<AlertDialogTitle>{t("workspace.virtualKeys.resetBudgetUsageTitle")}</AlertDialogTitle>
					<AlertDialogDescription>{t("supplemental.budgetUsageChangeDescription", { owner: ownerLabel })}</AlertDialogDescription>
				</AlertDialogHeader>
				<AlertDialogFooter>
					<AlertDialogCancel onClick={() => onChoice(false)} data-testid={testId ? `${testId}-preserve-btn` : undefined}>
						{t("supplemental.preserveUsage")}
					</AlertDialogCancel>
					<AlertDialogAction onClick={() => onChoice(true)} data-testid={testId ? `${testId}-confirm-btn` : undefined}>
						{t("supplemental.resetUsage")}
					</AlertDialogAction>
				</AlertDialogFooter>
			</AlertDialogContent>
		</AlertDialog>
