...
function StatusFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		<FilterSection title={i18n.t("workspace.virtualKeys.status")} defaultOpen={defaultOpen || hasActive} testId="status-filter-toggle">
			{Statuses.map((status) => (
				<CheckboxFilterItem
					key={status}
...
function ProvidersFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		<FilterSection
			title={i18n.t("sidebar.sub.providers")}
			defaultOpen={defaultOpen || hasActive}
			loading={isLoading}
			onOpenChange={setOpened}
...
function TypeFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		<FilterSection title={i18n.t("workspace.logs.colType")} defaultOpen={defaultOpen || hasActive} testId="type-filter-toggle">
...
function ModelsFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		<FilterSection
			title={i18n.t("sidebar.nav.models")}
			defaultOpen={defaultOpen || hasActive}
			loading={isLoading}
			onOpenChange={setOpened}
...
function RoutingRulesFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		<FilterSection
			title={i18n.t("sidebar.sub.routingRules")}
			defaultOpen={defaultOpen || hasActive}
			loading={isLoading}
			onOpenChange={setOpened}
...
function TeamFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		<FilterSection
			title={i18n.t("sidebar.sub.teams")}
			defaultOpen={defaultOpen || hasActive}
			loading={isLoading}
			onOpenChange={setOpened}
...
function CustomerFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		<FilterSection
			title={i18n.t("sidebar.sub.customers")}
			defaultOpen={defaultOpen || hasActive}
			loading={isLoading}
			onOpenChange={setOpened}
...
function LatencyFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	const updateSeconds = (field: "min_latency" | "max_latency", value: string) => {
		...
		const milliseconds = parsed === undefined || !Number.isFinite(parsed) || parsed < 0 ? undefined : Math.round(parsed * 1000);
		const next = {
			...filters,
			[field]: milliseconds,
		};
	...
	return (
		<FilterSection
			title={i18n.t("workspace.logs.latencyFilter.title")}
			defaultOpen={defaultOpen || hasActive}
			testId="latency-filter-toggle"
		>
			<div className="space-y-3 p-3">
				<div className="flex flex-wrap gap-1.5">
					{latencyPresets.map((seconds) => {
						...
						return (
							...
							>
								{i18n.t("workspace.logs.latencyFilter.preset", { seconds })}
							</Button>
						);
					})}
				</div>
				<div className="grid grid-cols-2 gap-2">
					<label className="space-y-1 text-xs">
						<span className="text-muted-foreground">{i18n.t("workspace.logs.latencyFilter.minimum")}</span>
						<Input
							type="number"
							min={0}
							step={1}
					...
					<label className="space-y-1 text-xs">
						<span className="text-muted-foreground">{i18n.t("workspace.logs.latencyFilter.maximum")}</span>
						<Input
							type="number"
							min={0}
							step={1}
							...
							placeholder={i18n.t("workspace.logs.latencyFilter.noLimit")}
							data-testid="latency-filter-max-seconds"
						/>
					</label>
				</div>
				<div className="flex items-center justify-between gap-2">
					<p className="text-muted-foreground text-xs">{i18n.t("workspace.logs.latencyFilter.unitHelp")}</p>
					{hasActive && (
						<Button
							type="button"
							variant="ghost"
						...
						>
							{i18n.t("workspace.logs.latencyFilter.clear")}
						</Button>
					)}
				</div>
			</div>
...
function CostFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		<FilterSection title={i18n.t("workspace.logs.colCost")} defaultOpen={defaultOpen || hasActive} testId="cost-filter-toggle">
			<CheckboxFilterItem
