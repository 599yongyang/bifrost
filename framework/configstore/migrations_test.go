...
func TestMigrationAddErrorFallbacksColumnToRoutingRules(t *testing.T) {
	...
	require.NoError(t, db.Exec(`
		CREATE TABLE routing_rules (
			...
			chain_rule BOOLEAN NOT NULL DEFAULT 0,
	...
	var gotHash string
	require.NoError(t, db.Table("routing_rules").Select("config_hash").Where("id = ?", "route-1").Scan(&gotHash).Error)
	assert.NotEmpty(t, gotHash)
	assert.NotEqual(t, "stale-hash", gotHash)
}
