...
func TestAlertCooldownUpsertAndChannelDetach(t *testing.T) {
	...
	settings := &tables.TableDailyReportSettings{
		...
		InternalChannelIDs: []string{"c-internal"},
		ExternalChannelIDs: []string{"c-external"},
	}
	...
	if loadedSettings.ID != tables.DefaultDailyReportSettingsID ||
		loadedSettings.Timezone != "Asia/Shanghai" ||
		loadedSettings.GenerateTime != "03:00" ||
		loadedSettings.SendTime != "09:00" ||
		loadedSettings.SlowThresholdMs != 15000 ||
		len(loadedSettings.InternalChannelIDs) != 1 ||
		loadedSettings.InternalChannelIDs[0] != "c-internal" ||
		len(loadedSettings.ExternalChannelIDs) != 1 ||
		loadedSettings.ExternalChannelIDs[0] != "c-external" {
