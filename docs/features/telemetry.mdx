...
<Warning>
Legacy `x-bf-prom-*` headers are removed in v2 and are discarded. Migrate callers to `x-bf-dim-*`
before upgrading so label dimensions do not silently disappear.
</Warning>
