...
| `x-moon-trace-id` | The trace ID linking all spans for the request. Inherited from an incoming W3C `traceparent` when present, otherwise generated. |
...
## Removed Prometheus-only headers

The legacy `x-bf-prom-*` header family is removed in v2. These headers are discarded and do not
create Prometheus labels or Bifrost context values. Use the canonical [`x-bf-dim-*`](#unified-dimensions-x-bf-dim) headers instead; unified dimensions feed Prometheus, logs, OpenTelemetry, and Maxim together.
