...
> **当前服务器部署入口：[多节点部署、发布与回退手册](DEPLOYMENT.md)。**
> 生产环境现已采用 Compose＋共享 PostgreSQL，并配置独立测试/生产备用模式。
> 本文保留 fork 构建和历史维护背景；实际服务器路径、测试接管顺序、插件文件级挂载、
> 滚动升级与数据库回退边界，以新手册为准。不要直接沿用旧的单机 SQLite/Canary 命令。
