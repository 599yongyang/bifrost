---
title: "Alert History"
description: "Review delivered alert notifications, coalesced suppressions, and failed delivery attempts in Bifrost."
...
Bifrost records matched delivery outcomes and evaluation failures in **alert history**. Each record captures the rule, scope and target, relevant metric values, delivery channel when applicable, and outcome status. Ordinary non-matches are not stored.
...
Bifrost uses its shared key-value store for live rule cooldown and reset-cycle checks. After startup or a leadership change, the alerting engine bulk-loads only the latest relevant `sent` timestamps and reset-cycle identities from history before it can dispatch alerts. Normal evaluation sweeps do not query alert history for each rule.
...
| **Channel** | The channel that was notified, or a rule-level suppression marker for cooldown skips. |
...
| `skipped` | The rule matched but delivery was suppressed. Consecutive scans in the same cooldown or reset cycle are coalesced into one record. |
...
Each history record stores only metrics relevant to its rule type. Internal scope identifiers and zero-filled metrics from the other rule family are omitted. Example for a budget alert:
...
| `channel_type` | string (comma-separated) | Filter by channel: `slack`, `microsoft_teams`, `wecom`, `pagerduty`, `webhook`. |
