...
func (m *Manager) GetDailyReportSettings(ctx context.Context) (*tables.TableDailyReportSettings, error) {
	if m.dailySettings == nil {
		return nil, fmt.Errorf("daily reports require a settings store")
	}
	settings, err := m.dailySettings.GetDailyReportSettings(ctx)
...
func (m *Manager) UpdateDailyReportSettings(ctx context.Context, settings *tables.TableDailyReportSettings) (*tables.TableDailyReportSettings, error) {
	...
	if err := m.dailySettings.UpsertDailyReportSettings(ctx, normalized); err != nil {
		return nil, err
	}
...
func (m *Manager) PreviewDailyReport(ctx context.Context, settings *tables.TableDailyReportSettings, businessDate string) (*DailyReportPreview, error) {
	preview, activeSettings, resolvedDate, found, err := m.CachedDailyReportPreview(ctx, settings, businessDate)
	if err != nil || found {
		return preview, err
	}
...
// CachedDailyReportPreview only reads an already frozen report. It never scans
// request logs, so HTTP handlers can safely use it on the request goroutine.
func (m *Manager) CachedDailyReportPreview(ctx context.Context, settings *tables.TableDailyReportSettings, businessDate string) (*DailyReportPreview, *tables.TableDailyReportSettings, string, bool, error) {
	...
	if m.dailyReports != nil {
		...
		if findErr == nil && (existing.InternalContent != "" || existing.ExternalContent != "") {
			...
			}, activeSettings, resolvedDate, true, nil
		}
		if findErr != nil && !errors.Is(findErr, logstore.ErrNotFound) {
			return nil, nil, "", false, findErr
		}
	}
	return nil, activeSettings, resolvedDate, false, nil
}
...
func NewDailyReportJobMetadata(
	...
	trigger, err = normalizeDailyReportTrigger(trigger)
	if err != nil {
		return "", "", err
	}
...
func (m *Manager) RunDailyReportJob(
...
) (finalMetadata string, finalErr error) {
	...
	normalizedTrigger, err := normalizeDailyReportTrigger(meta.Trigger)
	if err != nil {
		return "", err
	}
	meta.Trigger = normalizedTrigger
	...
	meta.RunID = dailyReportRunID(settings.Timezone, resolvedDate)
	defer func() {
		if recovered := recover(); recovered != nil {
			if m.logger != nil {
				m.logger.Error("alerting: recovered daily report job panic panic_type=%T\n%s", recovered, debug.Stack())
			}
			now := time.Now().UTC()
			if m.dailyReports != nil {
				if run, err := m.dailyReports.FindDailyReportRun(context.WithoutCancel(ctx), meta.RunID); err == nil {
					...
					run.InternalStatusDetail = "daily report job failed unexpectedly"
					run.ExternalStatusDetail = "daily report job failed unexpectedly"
					run.CompletedAt = &now
					_ = m.updateDailyReportRun(context.WithoutCancel(ctx), run)
				}
			}
			meta.Stage = "failed"
			meta.Message = "Daily report job failed unexpectedly"
			if encoded, err := sonic.Marshal(meta); err == nil {
				finalMetadata = string(encoded)
			}
			finalErr = fmt.Errorf("daily report job failed unexpectedly")
		}
	}()
	...
	result, err := m.generateDailyReportWithProgress(ctx, settings, resolvedDate, meta.Trigger, meta.Deliver, metricsProgress)
	if err != nil {
		return "", err
	}
...
func normalizeDailyReportTrigger(trigger string) (string, error) {
	trigger = strings.ToLower(strings.TrimSpace(trigger))
	if trigger == "" {
		return "manual", nil
	}
	if trigger != "manual" && trigger != "scheduled" {
		return "", fmt.Errorf("daily report trigger must be manual or scheduled")
	}
	return trigger, nil
}
...
func (m *Manager) maybeDispatchScheduledDailyReport(ctx context.Context) error {
	...
	case findErr == nil:
		...
		if run.Status == logstore.DailyReportRunStatusRunning && m.currentTime().Sub(run.StartedAt) >= 15*time.Minute && nowLocal.Before(generateAt.Add(dailyReportGenerationGracePeriod)) {
			if m.dailyReportJobs != nil {
				return m.dailyReportJobs(ctx, businessDate, false, settings)
			}
		}
	...
	return m.dailyReportJobs(ctx, businessDate, true, settings)
}
