...
func newHTTPClient(allowPrivate bool) *http.Client {
	...
	if allowPrivate {
		transport.DialContext = corenetwork.SSRFSafeDialContextAllowPrivate(5 * time.Second)
	} else {
...
func blockedHeader(key string) bool {
	...
	blocked := []string{"authorization", "connection", "content-length", "cookie", "host", "proxy-authorization", "set-cookie", "te", "trailer", "transfer-encoding", "upgrade"}
	return slicesContains(blocked, key)
}
