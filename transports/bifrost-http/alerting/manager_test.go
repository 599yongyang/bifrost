...
type panicOnceAlertStore struct {
	*memoryAlertStore
	calls atomic.Int32
}

func (s *panicOnceAlertStore) ListAlertRules(ctx context.Context) ([]tables.TableAlertRule, error) {
	if s.calls.Add(1) == 1 {
		panic("secret store panic")
	}
	return s.memoryAlertStore.ListAlertRules(ctx)
}
...
func TestSweepRecoversEvaluationPanicAndContinues(t *testing.T) {
	base := &memoryAlertStore{}
	store := &panicOnceAlertStore{memoryAlertStore: base}
	manager, err := NewManager(store, nil, nil, base, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	manager.sweepInterval = 10 * time.Millisecond
	...
	if store.calls.Load() < 2 {
		t.Fatalf("sweep stopped after recovered panic, calls=%d", store.calls.Load())
	}
	...
	case <-time.After(time.Second):
		t.Fatal("Close hung after recovered sweep panic")
	}
}

func TestDeliveryPanicIsContained(t *testing.T) {
	store := &memoryAlertStore{}
	manager, err := NewManager(store, nil, nil, store, nil, &Config{WebhookNetwork: NetworkConfig{AllowHTTP: true}})
	if err != nil {
		t.Fatal(err)
	}
	manager.client = &http.Client{Transport: roundTripFunc(func(*http.Request) (*http.Response, error) {
		panic("secret delivery panic")
	})}
	rule := &tables.TableAlertRule{ID: "rule", Name: "rule"}
	channel := &tables.TableAlertChannel{ID: "channel", Name: "channel", Type: tables.AlertChannelWebhook, Config: map[string]any{"url": "http://example.com/hook"}}
	if err := manager.deliver(context.Background(), rule, channel, map[string]any{}, time.Now()); err == nil || strings.Contains(err.Error(), "secret") {
		t.Fatalf("contained delivery error = %v", err)
	}
}
...
func TestWeComDeliveryUsesDynamicMarkdownAndChecksErrorCode(t *testing.T) {
	...
	manager.privateClient = &http.Client{Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
		return &http.Response{StatusCode: 200, Body: io.NopCloser(strings.NewReader(`{"errcode":93000,"errmsg":"` + secretResponse + `"}`)), Header: make(http.Header)}, nil
	})}
	if err := manager.deliver(context.Background(), rule, channel, input, time.Now()); err == nil || !strings.Contains(err.Error(), "errcode 93000") || strings.Contains(err.Error(), secretResponse) {
		t.Fatalf("expected WeCom API error, got %v", err)
	}
}

func TestWebhookDeliverySendsAuthorizationAndDropsUnsafeHeaders(t *testing.T) {
	store := &memoryAlertStore{}
	manager, err := NewManager(store, nil, nil, store, nil, &Config{WebhookNetwork: NetworkConfig{AllowHTTP: true, AllowPrivateNetwork: true}})
	if err != nil {
		t.Fatal(err)
	...
	manager.privateClient = &http.Client{Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
		if got := req.Header.Get("Authorization"); got != "Bearer webhook-secret" {
			t.Fatalf("authorization header = %q", got)
		}
		if got := req.Header.Get("X-Tenant"); got != "tenant-1" {
			t.Fatalf("custom header = %q", got)
		}
		for _, key := range []string{"Connection", "Content-Length", "Proxy-Connection", "Keep-Alive", "TE", "Trailer", "Transfer-Encoding", "Upgrade"} {
			if got := req.Header.Get(key); got != "" {
				t.Fatalf("blocked header %s was sent as %q", key, got)
			}
		}
		return &http.Response{StatusCode: http.StatusOK, Body: io.NopCloser(strings.NewReader("ok")), Header: make(http.Header)}, nil
	})}
	rule := &tables.TableAlertRule{ID: "r1", Name: "test", ScopeType: "system", ScopeID: "bifrost", CELExpression: "true"}
	channel := &tables.TableAlertChannel{ID: "c1", Type: tables.AlertChannelWebhook, Config: map[string]any{
		"url": "http://127.0.0.1/hook",
		"headers": map[string]any{
			"Authorization": "Bearer webhook-secret", "X-Tenant": "tenant-1", "Connection": "close", "Content-Length": "1",
			"Proxy-Connection": "keep-alive", "Keep-Alive": "timeout=5", "TE": "trailers", "Trailer": "X-Test", "Transfer-Encoding": "chunked", "Upgrade": "websocket",
		},
	}}
	if err := manager.deliver(context.Background(), rule, channel, map[string]any{}, time.Now()); err != nil {
		t.Fatal(err)
	}
}

func TestWeComFailureHistoryDoesNotLeakResponseMessage(t *testing.T) {
	const secretResponse = "secret webhook token"
	store := &memoryAlertStore{
		channels: []tables.TableAlertChannel{{ID: "c1", Name: "WeCom", Type: tables.AlertChannelWeCom, Enabled: true, Config: map[string]any{"webhook_url": "http://127.0.0.1/hook"}}},
		rules:    []tables.TableAlertRule{{ID: "r1", Name: "Always", Enabled: true, ScopeType: "provider", ScopeID: "openai", CELExpression: "provider_error_rate >= 0.0", ChannelIDs: []string{"c1"}, WindowSeconds: 300, MinRequests: 1}},
	}
	metrics := &fakeMetricsStore{total: 1, errors: 1}
	manager, err := NewManager(store, nil, metrics, store, nil, &Config{WebhookNetwork: NetworkConfig{AllowHTTP: true, AllowPrivateNetwork: true}})
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(store.history) != 1 || store.history[0].Status != "failed" {
		t.Fatalf("unexpected history: %#v", store.history)
	}
	if detail := store.history[0].StatusDetail; strings.Contains(detail, secretResponse) || detail != "WeCom webhook rejected the message (errcode 93000)" {
		t.Fatalf("unsafe history detail: %q", detail)
	}
}
