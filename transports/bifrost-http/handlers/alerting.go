...
type AlertingHandler struct {
	...
	dailyReportRunner   *sidekiq.Runner
	dailyReportJobStore DailyReportJobStore
}
...
func (h *AlertingHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.BifrostHTTPMiddleware) {
	...
	r.POST("/api/alerting/reports/jobs", lib.ChainMiddlewares(h.startDailyReportJob, middlewares...))
	r.GET("/api/alerting/reports/jobs/status", lib.ChainMiddlewares(h.getDailyReportJobStatus, middlewares...))
...
func (h *AlertingHandler) testChannel(ctx *fasthttp.RequestCtx) {
	...
	if err := h.manager.TestChannel(ctx, channel); err != nil {
...
func (h *AlertingHandler) evaluateNow(ctx *fasthttp.RequestCtx) {
	if err := h.manager.EvaluateNow(ctx); err != nil {
