...
func (s *BifrostHTTPServer) RegisterAPIRoutes(ctx context.Context, callbacks ServerCallbacks, middlewares ...schemas.BifrostHTTPMiddleware) error {
	...
	// Provider reliability rules only depend on LogsStore, so alerting remains
	// available even when the governance plugin is not loaded. Governance-scoped
	// rules still validate fail-closed when their snapshot is unavailable.
